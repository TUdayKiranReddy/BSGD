...
def add_noise(y, snr, mu=0, device="cpu", constant_snr=True):
    if snr == np.inf:
        return y
...
def gradient(df, phi, i=None, approx=None, snr=np.inf, mu=0, c=1e-6, batch_size=64, device="cpu"):
...
def simulate(grad, f, GD, approx=1, mu_noise=0, snr=np.inf, is_BCD=False, delta=0.5, is_require_coords=False, batch_size=1, scheduler=True, ITR_LIM=1000, step=1, seed=None, load_phi=False, return_params=False, tau=2e2, phi=None, p=1, q=0.02, c=None, N=1000, isDNN=False):
    if load_phi:
    ...
    values_0 = [f(opt.phi, device=device).cpu().numpy()]
    print("Initial Value: {} Optimal Value: {} #Params: {}".format(values_0[0], 0, phi.shape[0]))
    params_c = []
    params_eps = []
    ...
    for i in tqdm(range(0, ITR_LIM, step)):
        ...
        if return_params:
            params_c.append(opt.gamma if hasattr(opt, 'gamma') else opt.c)
            params_eps.append(opt.eps)
        values_0.append(f(opt.phi, device=device).cpu().numpy())
